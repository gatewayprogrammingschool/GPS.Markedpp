            ...
            if (markedppOptions.rootSourceFiles.length > 0) {
              markedppOptions.rootSourceFiles.forEach(sourceFile => {
                ...
                var path = filename.substring(
                  0,
                  filename.length - filename.replace(/^.*[\\\/]/, "").length
                );
                ...
                destination = require("path").join(
                  rootFolder,
                  markedppOptions.outputDirectory
                );
      ...
      vscode.workspace.openTextDocument(path).then(
        document => {
          ...
          try {
            ...
            json.rootSourceFiles.forEach((element: string) =>
        ...
        error => {
          vscode.window.showErrorMessage(
            "Could not process configuration. " + error
          );
          reject(error);
        }
      );
